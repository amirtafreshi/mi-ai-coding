 ...
 * Run with: DISPLAY=:99 node tests/manual/test-websocket-browser.js
 */
...
async function createActivityLog(page, agent, action, details, level = 'info') {
...
async function main() {
  ...
  try {
    ...
    // Use ID selectors instead of name
    await page.fill('#basic_email', 'admin@example.com')
    await page.fill('#basic_password', 'admin123')
